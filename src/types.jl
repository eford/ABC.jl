...
""" 
   abc_pmc_plan_type(gd::Function,css::Function,cd::Function,p::Distribution;
     optional params)
...
make_proposal_dist: Generate proposal for new set of parameters
param_active::Vector{Int64}: Which parameters are being updated by ABC simulation (default: all)
normalize::Function:  Function to update model parameters generated by make_proposal_dist (default: noop)
is_valid::Function:   Function to test whether proposed parameters are valid (default: noop)
num_part::Integer:   Number of particles for sequential importance sampling (default: 10*(number of active parameters)^2
num_max_attempt:     Maximum number of attempts to draw a set of parameters (for a single particle and single generation) while trying to generate parameter values that are accepted  (default: 1000)
num_max_attempt_value: Maximum number of attempts to draw a set of parameters that result in valid parameters, while trying to generate parameter values that are accepted (default: 50)
num_max_times:      Maximum number of generations for sequential importance sampler (default: 100)
num_dist_per_obs:   Number of times to compute the distance for each set of simulated observations.  Useful for stochastic distace functios (default: 1)
epsilon_init:       Initial values of epsilon, maximum distance to be accepted
init_epsilon_quantile: After initial generation, will next epsilon is based on this quantile of the epsilon distribution (default: 0.75)
epsilon_reduction_factor: Factor used for reducing epsilon each generation (default: 0.90)
target_epsilon:    Value of epsilon at which ABC simulation halts (default: 0.01)
tau_factor:        Used by some proposals to control how much importance sampling density should be overdispersed relative to current population of parameters (default: 2)
adaptive_quantiles::Bool:  Whether to attempt to reduce epsilon based on some fancy algorithm (default: false)
stop_on_decreasing_efficiency::Bool: Whether to halt simulation once efficiency starts decreasing
save_params::Bool:  Whether to save history of model parameters (default: true)
save_summary_stats::Bool: Whether to save history of summaray stats (default: false)
save_distances::Bool: Whether to save history of distances (default: true)
frac_rejects_save:  What fraction of rejected parameters/summary statistics/distances should be saved in history (default: 0.0)
in_parallel::Bool:  Whether to parallelize ABC simulation over particles (default: false) <- Warning: Need to fix
"""
