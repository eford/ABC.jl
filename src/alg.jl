...
function run_abc(plan::abc_pmc_plan_type, ss_true, pop::abc_population_type; verbose::Bool = false, print_every::Integer=1, in_parallel::Bool = plan.in_parallel )
  ...
  for t in 1:plan.num_max_times
    ...
    if verbose && (t%print_every == 0)
       ...
       println("Mean(theta)= ", reshape(mean(pop.theta, 2), (1,size(pop.theta, 1))), " Stand. Dev.(theta)= ", reshape(std(pop.theta, 2), (1, size(pop.theta, 1))))
    ...
    push!(mean_arr, reshape(mean(pop.theta, 2), (1,size(pop.theta, 1))))
    push!(std_arr, reshape(std(pop.theta, 2), (1, size(pop.theta, 1))))
