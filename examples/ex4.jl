...
function calc_summary_stats_mean_stddev_detected(data::Tuple{Array{Float64,2},BitArray{1}})
  ...
  @assert size(period_snr,2) == length(detected) >= 2
  (m, c)  = mean_and_cov(log(period_snr[:,detected]))
  return [vec(m)[1], vec(m)[2], sqrt(c[1,1]), sqrt(c[2,2]), c[1,2] ]
  #=
  ...
  return [mean_log_period, stddev_log_period, mean_log_snr, stddev_log_snr]
  =#
end
...
# Uncomment one of below one to pick how data is generated
...
ex_data =gen_data(theta_true,100)
...
abc_plan = abc_pmc_plan_type(gen_data,calc_summary_stats_mean_stddev_detected,ABC.calc_dist_max, param_prior;
                             ...
                             target_epsilon=0.01,num_part=50,
