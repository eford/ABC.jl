...
param_prior_rate = Gamma(1.0,1.0)
 ...
 function is_valid_model(theta::Array{Float64,1})
    ...
    return is_valid_mean_covar(theta)
 end
...
function calc_mean_summary_stats_mean_stddev_rate_period_snr_detected(data::Tuple{Array{Float64,2},BitArray{1}})
  ...
  if num_detected<2
     return [sqrt(lambda),mean_log_period,mean_log_snr,Inf,Inf,Inf][1:num_param_active]
  end
  ...
  return [sqrt(lambda), mean_log_period, mean_log_snr, stddev_log_period, stddev_log_snr, cor_F][1:num_param_active]
end
...
function calc_stddev_summary_stats_mean_stddev_rate_period_snr_detected(ss::Array{Float64,1} )
  ...
  return [0.5*one_over_sqrtn, sd_logp*one_over_sqrtn, sd_logsnr*one_over_sqrtn, sd_logp*two_to_quarter*one_over_sqrtn, sd_logsnr*two_to_quarter*one_over_sqrtn, 1/sqrt(max(1,n-3))][1:num_param_active]
end
...
abc_plan = ABC.abc_pmc_plan_type(gen_data,
                                 ...
                                 ABC.calc_scaled_dist_max, param_prior;target_epsilon=6.0,epsilon_init=300.0,num_part=60,
