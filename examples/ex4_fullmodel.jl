...
function calc_stddev_summary_stats_mean_stddev_rate_period_snr_detected(data::Tuple{Array{Float64,2},BitArray{1}}, ss::Array{Float64,1} )
  n = num_star_default*ss[1]
  sd_logp = ss[3]
  sd_logsnr = ss[5]
  return [sqrt(n)/num_star_default, sd_logp/sqrt(n), sd_logp/n^(1//4), sd_logsnr/sqrt(n), sd_logsnr/n^(1//4), 1/sqrt(n)]
end
...
function calc_mean_stddev_summary_stats_mean_stddev_rate_period_snr_detected(data::Tuple{Array{Float64,2},BitArray{1}})
  ...
  #return [ mean_stddev_type(mean_ss[i],stddev_ss[i]) for i in 1:length(mean_ss) ]
...
abc_plan = ABC.abc_pmc_plan_type(gen_data,
                                 ...
                                 ABC.calc_scaled_dist_max, param_prior;target_epsilon=1.0,epsilon_init=300.0,num_part=40,
...
#@time pop_out = run_abc_emulated(abc_plan,ss_true, verbose=true);
